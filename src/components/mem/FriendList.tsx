...
import { apiMergeFriend, apiSearchFriendList } from '@/service/mem/apiMemFriend';
      ...
      <div className="flex items-center justify-center min-h-screen bg-background p-0 md:p-4">
        <div className="flex flex-col w-full h-[100dvh] md:h-[600px] md:max-w-md md:rounded-lg md:border md:shadow-lg bg-card overflow-hidden">
          <BackHeader
            title={
              tab === 'friend'
                ? `친구목록(${data?.friendList?.filter((obj) => obj.blockYn === 'N').length})`
                : `차단목록(${data?.friendList?.filter((obj) => obj.blockYn === 'Y').length})`
            }
            menu
            menuProps={menuProps}
          />
            ...
            {tab === 'friend' ? (
              <>
                {data &&
                  ...
                  data.friendList.filter((obj) => obj.blockYn === 'N').length > 0 &&
                  data.friendList
              ...
              <>
                {data &&
                  ...
                  data.friendList.filter((obj) => obj.blockYn === 'Y').length > 0 &&
                  data.friendList
